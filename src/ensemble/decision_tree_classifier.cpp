...
namespace ml {
DecisionTreeClassifier::DecisionTreeClassifier(nlohmann::json parameters,
  ...
  if (parameters.contains("impurity_method")) {
    ...
    if (impurity_method_input != "gini" && impurity_method_input != "entropy")
      logger->log(WARNING,
                  "The specified method of calculating node impurity, " +
                      impurity_method_input +
                      " is currently unsupported. The default gini method will "
                      "be used instead.");
    else
      impurity_method = impurity_method_input;
  }

  if (parameters.contains("search_algorithm")) {
    string search_algorithm_input = parameters["search_algorithm"];
    if (search_algorithm_input != "breadth" &&
        search_algorithm_input != "depth")
      logger->log(WARNING, "The specified method of growing the tree, " +
                               search_algorithm_input +
                               " is currently unsupported. The default breadth "
                               "first search method will be used instead.");
    else
      search_algorithm = "depth";
  }

  if (parameters.contains("max_feature_fraction")) {
    double max_feature_fraction_input = parameters["max_feature_fraction"];
    if (max_feature_fraction_input < 0.2)
      max_feature_fraction = 0.2;
    else if (max_feature_fraction > 1.0)
      max_feature_fraction = 1.0;
    else
      max_feature_fraction = max_feature_fraction_input;
  }
}

vector<unordered_map<double, int, DoubleHash, DoubleEqual>>
DecisionTreeClassifier::get_classes_frequencies(const vector<int> &indices) {
  ...
  for (int i = 0; i < number_output_variables; i++) {
    ...
    for (int index : indices) {
      ...
      if (current_frequencies.count(current_value))
        current_frequencies[current_value]++;
      else
        current_frequencies.insert(std::make_pair(current_value, 1));
    }
...
double DecisionTreeClassifier::get_impurity(const vector<int> &indices) {
  ...
  vector<unordered_map<double, int, DoubleHash, DoubleEqual>>
      classes_frequencies = get_classes_frequencies(indices);
  ...
  if (impurity_method == "gini") {
    for (int i = 0; i < number_outputs; i++) {
      unordered_map<double, int, DoubleHash, DoubleEqual> current_frequencies =
          classes_frequencies[i];
      double output_impurity = 0.0;
      for (auto it = current_frequencies.begin();
           it != current_frequencies.end(); it++) {
    ...
    for (int i = 0; i < number_outputs; i++) {
      ...
      for (auto it = current_frequencies.begin();
        ...
        double class_probability = 1.0 * (it->second) / number_instances;
        output_impurity +=
            -1.0 * class_probability * log(1.0 - class_probability);
      }
      mean_impurity += output_impurity;
    }
  }
...
DecisionTreeClassifier::split_node(shared_ptr<TreeNode> node) {
  ...
  for (int i = 0; i < number_features; i++)
    feature_indices[i] = i;
  std::shuffle(feature_indices.begin(), feature_indices.end(),
               random_generator);
  ...
  for (int i = 0; i < max_features; i++) {
    ...
    for (int data_index : data_indices)
      unique_values.insert(train_features[data_index][feature_index]);
    ...
    for (double value : unique_values) {
      for (int data_index : data_indices) {
        ...
        if (current_value <= value)
          left_indices.push_back(data_index);
        else
          right_indices.push_back(data_index);
      }
      ...
      double average_child_impurity =
          (1.0 * left_indices.size() / data_indices.size()) * left_impurity +
          (1.0 * right_indices.size() / data_indices.size()) * right_impurity;
  ...
  if (best_feature_split > -1) {
    shared_ptr<TreeNode> left_child(
        new TreeNode(best_left_indices, node->depth + 1));
    shared_ptr<TreeNode> right_child(
        new TreeNode(best_right_indices, node->depth + 1));
    ...
    number_splits++;
    feature_importances[best_feature_split] +=
        (1.0 * data_indices.size() / train_features.size()) *
        max_impurity_reduction;
  } else
    number_leaf_nodes++;
 ...
 */
void DecisionTreeClassifier::set_data(TrainTestData &&train_test) {
...
void DecisionTreeClassifier::fit() {
  ...
  logger->log(DEBUG, "Number of training instances = " +
                         to_string(train_features.size()));

  logger->log(DEBUG,
              "Number of features = " + to_string(train_features[0].size()));
  logger->log(DEBUG,
              "Number of outputs = " + to_string(train_labels[0].size()));
  feature_importances.resize(number_features);
  fill(feature_importances.begin(), feature_importances.end(), 0.);
  if (impurity_method == "breadth")
    this->breadth_first_search();
  else
    this->depth_first_search(root);
  double feature_importances_sum =
      accumulate(feature_importances.begin(), feature_importances.end(), 0.);
  for (int i = 0; i < number_features; i++)
    feature_importances[i] /= feature_importances_sum;
  report_fit_results();
}
...
DecisionTreeClassifier::predict() {
  if (this->root == nullptr) {
    logger->log(ERROR, "Decision tree must be grown by calling fit method "
  ...
  for (int j = 0; j < number_test_instances; j++) {
    ...
    vector<unordered_map<double, int, DoubleHash, DoubleEqual>>
        classes_frequencies =
            get_classes_frequencies(current_node->node_indices);
...
void DecisionTreeClassifier::evaluate() {
...
void DecisionTreeClassifier::report_fit_results() {
  ...
  for (int i = 0; i < feature_importances.size(); i++)
    logger->log(INFO, "Feature " + to_string(i) +
                          " importance = " + to_string(feature_importances[i]));
}
} // namespace ml
