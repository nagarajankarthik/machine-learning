...
namespace ml {
	...
	struct DoubleHash {
		...
		return std::hash<int64_t>()(static_cast<int64_t>(key * 1000000)); // Scale and hash
	    }
	};
	...
	class BaseModel {
		 ...
		 */
		virtual void fit(const vector<vector<double>> && features, const vector<vector<double>> && labels) = 0;

		/**
		 * Prototype for model inference
		 */
		virtual vector<vector<double>> predict(const vector<vector<double>> & test_features) = 0;
		 ...
		 */
		vector<vector<int>> get_confusion_matrix_single(const vector<vector<int>> &predictions, const vector<vector<int>> &test_outputs, unordered_set<int> unique_classes, int index_output);
		 ...
		 */
		void get_confusion_matrices(const vector<vector<double>> & test_predictions_double, const vector<vector<double>> & test_labels_double);

		/**
		 * Get root mean square error for each output variable.
		 */			
		void get_root_mean_square_error(const vector<vector<int>> & test_predictions, const vector<vector<int>> & test_labels);

		/**
		 * Prototype for model evaluation
		 */
		virtual void evaluate(const vector<vector<double>> & test_features, const vector<vector<double>> & test_labels) = 0;
