...
namespace ml {
	...
	BaseModel::BaseModel(nlohmann::json model_parameters, shared_ptr<Logger> logger): logger(logger) {
	...
	vector<vector<int>> BaseModel::double_to_int(const vector<vector<double>> & data) {
		int m = data.size();
		int n = data[0].size();
	...
	vector<unordered_set<int>> BaseModel::get_unique_classes(vector<vector<int>> outputs)
	{
	...
	vector<vector<int>> BaseModel::get_confusion_matrix_single(const vector<vector<int>> &predictions, const vector<vector<int>> &test_outputs, unordered_set<int> unique_classes, int index_output)
	{
	...
	void BaseModel::get_confusion_matrices(const vector<vector<double>> & test_predictions_double, const vector<vector<double>> & test_labels_double) {
		...
		vector<vector<int>> train_labels_int = double_to_int(train_labels);

		vector<unordered_set<int>> unique_classes = get_unique_classes(train_labels_int);
			vector<unordered_set<int>> unique_classes_test = get_unique_classes(test_labels);
		...
		for (int j = 0; j < number_outputs; j++) {
			vector<vector<int>> confusion_matrix = get_confusion_matrix_single(test_predictions, test_labels, unique_classes[j], j);
